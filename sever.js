...
const envCheck = Object.prototype.hasOwnProperty.call(argv, 'env')
if (envCheck && argv.env) {
    console.log('Env param found:', argv.env)
    require('dotenv').config({
        path: `${__dirname}/config/.env.${argv.env.toLowerCase()}`,
    })
} else {
...
const errorMsg =
    "<p style='color:red;font-size:16px;padding:15px;font-family:arial,helvetica;'>Node server couldn't find the index.html file. Check to see if you have built the dist folder.</p>"
...
app.get(/.*/, nocache, (req, res) => {
    if (html.text) {
        ...
        res.writeHead(500, {
            'Content-Type': 'text/html',
            'Content-Length': html.text.length,
        })
    }
    res.write(html.text)
    res.end()
...
if (httpsEnabled) {
    ...
    https.createServer(options, app).listen(nodePort, () => {
        ...
        console.log(
            `${process.env.VITE_APP_NAME} server is running under HTTPS on port ${nodePort} `
        )
    })
} else {
    app.listen(nodePort, () => {
        ...
        console.log('APP_PROPS:', appProps)
        console.log(
            `${process.env.VITE_APP_NAME} server is running under HTTP on port ${nodePort}`
        )
    })
}
...
function getAppHtml() {
    ...
    try {
        ...
        console.log(
            "Node server couldn't find the index.html file. Check to see if you have built the dist folder.",
            err
        )
    }
